...
columnDefinition
    : identifier r_type (COMMENT string)? (WITH properties)?
    ;
...
primaryExpression
    ...
    | qualifiedName '(' ASTERISK ')' r_filter? over?                                        #functionCall
    | qualifiedName '(' (setQuantifier? expression (',' expression)*)?
        (ORDER BY sortItem (',' sortItem)*)? ')' r_filter? over?                            #functionCall
    ...
    | CAST '(' expression AS r_type ')'                                                     #cast
    | TRY_CAST '(' expression AS r_type ')'                                                 #cast
...
r_type
    ...
    | ARRAY '<' r_type '>'
    | MAP '<' r_type ',' r_type '>'
    | ROW '(' identifier r_type (',' identifier r_type)* ')'
    | baseType ('(' typeParameter (',' typeParameter)* ')')?
    | INTERVAL from=intervalField TO to=intervalField
    ;

typeParameter
    : INTEGER_VALUE | r_type
    ;
